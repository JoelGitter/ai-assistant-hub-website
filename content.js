...
function debouncedSummarizePageAction() {
  if (summarizeInProgress) {
    console.log('[AI Assistant] Summarize already in progress, ignoring duplicate call');
    return;
  }
  ...
  summarizeTimeout = setTimeout(() => {
    ...
    console.log('[AI Assistant] Starting summarize action');
    summarizePageAction().finally(() => {
      summarizeInProgress = false;
      console.log('[AI Assistant] Summarize action completed');
    });
  }, 100); // 100ms debounce
}
