  ...
  (async () => {
    try {
      if (request.action === 'summarizePage') {
        ...
        const value = await fillField(request.apiKey, request.context, request.instruction, request.formality, request.removeEmDash);
...
async function fillField(apiKey, context, instruction, formality, removeEmDash) {
  ...
  if (formality) prompt += `\nFormality: ${formality}`;
  if (formality && formality.toLowerCase() === 'long') {
    prompt += `\nMake the answer longer and more elaborate, with extra detail.`;
  }
